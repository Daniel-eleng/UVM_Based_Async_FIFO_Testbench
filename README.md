...
| `bug-injection/gray-encode-broken`      | In `FIFO_gray_counter`, the binary-to-Gray conversion `(bin_next >> 1) ^ bin_next` had its `^` replaced with `\|`, breaking the core Gray-code property (exactly one bit changing per increment) for *both* the write and read pointers simultaneously. | **168 / 833 PASS, 665 FAIL.** The corrupted pointers desynchronized `full`/`empty` from real occupancy on both sides at once — 491 "queue already at DEPTH" writes and 16 reads observed while the software queue was already empty. |

**A consistent gap in the SVA layer, confirmed across two independent faults.** Neither branch triggered `SVA_FULL_X`/`SVA_EMPTY_X` or `SVA_FULL_EMPTY`, and the reason is the same both times: each injected fault produces a `full`/`empty` value that is *wrong but still well-formed* — a valid `0` or `1`, never `X`, and never simultaneously asserted with its counterpart. The current assertion set only checks for *impossible* states (unknown values, structural conflicts); it has no notion of *functionally incorrect but structurally valid* states, so it cannot distinguish a healthy FIFO from one whose occupancy tracking is silently wrong. Seeing the identical blind spot on two unrelated fault types (a broken comparison and a broken encoding) confirms this isn't a one-off gap — it's a real limitation of black-box structural checks, and a concrete argument for adding a genuinely behavioral property (e.g., "`full` must assert within `DEPTH` writes given no intervening reads") as a future improvement. The scoreboard's queue-based model, in contrast, caught both faults decisively.
